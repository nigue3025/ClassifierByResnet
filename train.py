...
if __name__ == '__main__':
    ...
    defaultModelPath = 'models/pytorch/'+str(datetime.datetime.now().strftime("%Y%m%d%H%M%S"))+".h5"
    defaultDataPath = 'data/BlurryMarkupAll/Sampling'
    show_model=False
    epochs=40
    data_dir = 'data/BlurryMarkupAll/Sampling'
    ...
    parser.add_argument('--training-data-path', required=False, type=str, default=data_dir,
    ...
    parser.add_argument('--epochs', required=False, type=int, default=epochs,
                        help='True or False (sample 12 images to display')
    parser.add_argument('--out-model', required=False, type=str, default=defaultModelPath,
                        help='output path of model weight')
    parser.add_argument('--show-model',required=False,type=bool,default=show_model,help="visualize model result(remote machine not supported)")
    ...
    #torch.save(model_ft.state_dict(), 'models/pytorch/resnet50_Ada_weightsV2.h5')
    torch.save(model_ft.state_dict(), out_model_path)
    print("Trained model is saved to " + out_model_path)
    if(show_model):
        visualize_model(model_ft,class_names)
